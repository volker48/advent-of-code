...
object Program {
  ...
  def execute(memory: Memory,
    ...
    instruction.opcode match {
      ...
      case _ =>
        instruction(memory, output) match {
          case InstructionResult(None, None) =>
            execute(memory, newPtr, relBase, output)
          case InstructionResult(None, Some(offset)) =>
            execute(memory, newPtr, relBase + offset, output)
          case InstructionResult(Some(jmpPtr), None) =>
            execute(memory, jmpPtr.toInt, relBase, output)
          case default =>

...
import com.marcusmccurdy.Day3.{Grid, updateGrid}
...
class Vec(val direction: String, val magnitude: Int) {

  def toPoint(prev: Point): Point = {
    direction match {
      case "U" => Point(prev.x, prev.y + magnitude)
      case "D" => Point(prev.x, prev.y - magnitude)
      case "L" => Point(prev.x - magnitude, prev.y)
      case "R" => Point(prev.x + magnitude, prev.y)
    }
...
object Day3 extends App {
  type Grid = Map[Point, Set[String]]
  val line1 =
  ...
  def updateGrid(p: Point, grid: Grid, lineId: String): Grid = {
    val idSet = grid get p match {
      case Some(idSet) => idSet + lineId
      case None        => Set(lineId)
    }
    grid + (p -> idSet)
  }
